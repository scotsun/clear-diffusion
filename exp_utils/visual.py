...
def feature_swapping_plot(
    z_c, z_s, x, vae: torch.nn.Module,
    img_size=32,
    save=False,
    name="feature_swapping",
    ...
    with torch.no_grad():
        ...
        paired_z = torch.cat(
            (
                z_c[:, None, :, :, :].repeat(1, n, 1, 1, 1),
                z_s[None, :, :, :, :].repeat(n, 1, 1, 1, 1),
            ),
            dim=2,
        ...
        final_grid = torch.cat(
            [
                torch.cat(
                    [torch.ones(3, img_size + 4, img_size + 4).to(device), vgrid], dim=1
                ),
        ...
        if save:
            ...
            filename = os.path.join(out_dir, f"{timestamp}_{name}.png")

            plt.figure(figsize=(12, 12))
            plt.imshow(final_grid.permute(1, 2, 0).detach().cpu().numpy())
            plt.axis("off")
            plt.tight_layout()
            plt.savefig(filename, dpi=dpi, bbox_inches="tight")
            plt.close()
            print(f"Saved feature swapping grid to {filename}")
        else:
            plt.figure(figsize=(12, 12))
            plt.imshow(final_grid.permute(1, 2, 0).detach().cpu().numpy())
            plt.axis("off")
