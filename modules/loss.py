...
class DenseSNN(nn.Module):
    def __init__(
        ...
        pool: str = "gap",  # For compatibility with trainer
        proj_dim: int = 128,
        use_proj: bool = True
    ):
        super().__init__()
    ...
    def forward(self, z: torch.Tensor, y: torch.Tensor, ps: bool = False):
        """
        Args:
            z: Feature maps (B, C, H, W)
            y: Labels (B,)
            ps: If True, pushes away same-label samples (style contrast)
        Returns:
            Scalar loss
        """
        b, c_in, w_f, h_f = z.shape
        s = w_f * h_f
        device = z.device
        
        # Reshape to patches: (B, C, H, W) -> (B, S, C)
        z_patches = z.view(b, c_in, s).transpose(1, 2).contiguous()
        
        # Projection head
        if self.use_proj:
            z_proj = self.proj(z_patches.reshape(-1, c_in))
            z = z_proj.reshape(b, s, self.proj_dim)
        else:
            z = z_patches
        
        # L2 normalize patches
        z = F.normalize(z, dim=-1)  # (B, S, D)
        z_flat = z.reshape(-1, z.size(-1))  # (B*S, D)
        
        # Compute patch→sample similarities with logsumexp pooling
        sim_matrix = torch.empty(b * s, b, device=device, dtype=z.dtype)
        for j in range(b):
            z_j = z[j]  # (S, D)
            # Similarity: (B*S, D) @ (D, S) = (B*S, S)
            sim_j = torch.mm(z_flat, z_j.t()) / self.log_tau.exp()
            # Logsumexp pooling across patches of sample j
            sim_matrix[:, j] = logsumexp(sim_j, dim=1)
        
        # Build masks
        eye = torch.eye(b, dtype=torch.bool, device=device)
        self_mask = eye.repeat_interleave(s, dim=0)  # (B*S, B)
        
        if ps:
            # Style contrast: push away same-label samples
            label_mask = ((y[None, :] != y[:, None]) & (~eye)).repeat_interleave(s, dim=0)
        else:
            # Content contrast: pull together same-label samples
            label_mask = ((y[None, :] == y[:, None]) & (~eye)).repeat_interleave(s, dim=0)
        
        # Mask self-similarities
        sim_matrix = sim_matrix.masked_fill(self_mask, float('-inf'))
        
        # SNN loss: -log(exp(pos) / sum(exp(all)))
        pos_sim = sim_matrix.masked_fill(~label_mask, float('-inf'))
        loss = -logsumexp(pos_sim, dim=1) + logsumexp(sim_matrix, dim=1)
        
        # Filter out NaN/Inf (anchors with no positives)
        return loss[torch.isfinite(loss)].mean()
