...
from modules.loss import SupCon, SNN, DenseSupCon, DenseSNN  # noqa
...
class CLEAR_VAEFirstStageTrainer(Trainer):
    ...
    def _train(self, dataloader: DataLoader, verbose: bool, epoch_id: int):
        ...
        beta = self.args["beta"]
        gamma_1 = self.args["gamma_1"]
        gamma_2 = self.args["gamma_2"]
            ...
            for batch_id, batch in enumerate(bar):
                ...
                if "dense" in self.contrastive_criterions:
                    ...
                    loss = (
                        rec_loss
                        + beta * kl_loss
                        + gamma_1 * (0.5 * con_loss + 0.5 * dense_con_loss)
                        + gamma_2 * (0.5 * ps_loss + 0.5 * dense_ps_loss)
                    )
                else:
                    dense_con_loss = torch.tensor(0.0, device=device)
                    dense_ps_loss = torch.tensor(0.0, device=device)
                    loss = rec_loss + beta * kl_loss + gamma_1 * con_loss + gamma_2 * ps_loss
